...
for Nk in Nks:
    ...
    #basis = best.Basis("Legendre", mode_p_max=p_max, polarisation_on=True, k_grid_size=Nk, use_tetraquad=False, use_precomputed_QQ=False,
    #                        precomputed_weights_path=f"data/CMB-BEST_precomputes/uniform_tetrapyd_cython_Nk_{Nk}.csv")
    #basis = best.Basis("Legendre", mode_p_max=p_max, polarisation_on=True, k_grid_size=Nk, use_tetraquad=False)
    ...
    #basis = best.Basis("Legendre2015", mode_p_max=p_max, polarisation_on=True)
    basis = best.Basis("SineLegendre1000", mode_p_max=p_max, polarisation_on=True)
...
#    np.save(f"data/CMB-BEST_precomputes/analytical_mode_bispectra_covariance_p_{p_max}.npy", analytic_cov)
    ...
    df["Marginal S/N"] = df["marginal_f_NL"] / df["marginal_sample_sigma"]

    print(df)
    #df.to_csv(f"data/full_trio_constraints_p_{p_max}_Nk_{Nk}.csv", float_format="%.18e")
    #df.to_csv(f"data/sinleg_full_trio_constraints_p_{p_max}_Nk_{Nk}.csv", float_format="%.18e")
