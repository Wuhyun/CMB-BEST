...
class Basis:
    ...
    def load_data(self, use_precomputed_QQ=False):
...
#        cdef double [::1] tmp = np.zeros(tetra_npts, dtype=np.dtype("d"))
    ...
    def compute_analytical_mode_bispectra_covariance(self):
        ...
        assert(np.allclose(ps[1:]+1, self.mode_indices[0,1:]))

        # Bispectrum covariance of x^p y^q z^r symmetrised over (p,q,r) over Tetrapyd(alpha,1)
        poly_cov = tetraquad.analytic_poly_cross_product_alpha_ns(ps, qs, rs, alpha, negative_power=negative_power)

        ## Define X = (2/(1-alpha))*x, so 2*alpha/(1-alpha) <= X <= 2/(1-alpha)
        #rescale_factor = (2 / (1. - alpha)) ** (ps + qs + rs)
        #poly_cov = poly_cov[:,:] * rescale_factor[np.newaxis,:] * rescale_factor[:,np.newaxis]
        #print("TEST:", (poly_cov-poly_cov.T)/poly_cov)
        ...
        # Integration volume measure factor
        poly_cov *= k_max **3

        # Now, we need to find transformation matrix which converts polynomials into our legendre basis
        trans_matrix_1d = np.identity(p_max)

        #trans_matrix_1d[0,0] = k_min                               # 0th mode is k_min * k^(n_s-2)
        trans_matrix_1d[0,0] = k_min * (k_max ** negative_power)    # 0th mode is k_min * k^(n_s-2)
        #trans_matrix_1d[0,0] = k_min * ((k_max * (1 - alpha) / 2) ** negative_power)    # 0th mode is k_min * k^(n_s-2)

        #kbar_of_k = np.polynomial.Polynomial([-(k_max+k_min)/(k_max-k_min), 2/(k_max-k_min)])
        kbar_of_x = np.polynomial.Polynomial([-(k_max+k_min)/(k_max-k_min), 2*k_max/(k_max-k_min)])
        #kbar_of_X = np.polynomial.Polynomial([-(k_max+k_min)/(k_max-k_min), 1])
        for p in range(1, p_max):
            leg_of_kbar = np.polynomial.Legendre([0]*(p-1) + [1])
            #leg_of_k = leg_of_kbar(kbar_of_k)
            #coef = leg_of_k.coef
            leg_of_x = leg_of_kbar(kbar_of_x)
            coef = leg_of_x.coef
            #leg_of_X = leg_of_kbar(kbar_of_X)
            #coef = leg_of_X.coef
            trans_matrix_1d[p,1:p+1] = coef[:]
        ...
        # First extend the matrix to a full (p_max**3, p_max**3) matrix
        p1, p2, p3 = self.mode_indices
        full_poly_cov = np.zeros((p_max ** 3, p_max ** 3))

        for pp1, pp2, pp3 in itertools.permutations([p1, p2, p3]):
            i_slice = pp1 * p_max ** 2 + pp2 * p_max + pp3
            for qq1, qq2, qq3 in itertools.permutations([p1, p2, p3]):
                j_slice = qq1 * p_max ** 2 + qq2 * p_max + qq3
                ij_slice = np.meshgrid(i_slice, j_slice, indexing="ij")
                full_poly_cov[ij_slice[0], ij_slice[1]] = poly_cov[:,:]
        
        # Compute the full transformation matrix
        trans_matrix_3d = np.zeros((p_max ** 3, p_max ** 3))
        f1, f2, f3 = np.meshgrid(np.arange(p_max), np.arange(p_max), np.arange(p_max), indexing="ij")
        f1, f2, f3 = f1.flatten(), f2.flatten(), f3.flatten()
        for t in range(p_max ** 3):
            trans_matrix_3d[t,:] = trans_matrix_1d[f1[t],f1] * trans_matrix_1d[f2[t],f2] * trans_matrix_1d[f3[t],f3]
        
        # Compute the full analytical covaraince
        full_analytic_cov = np.matmul(np.matmul(trans_matrix_3d, full_poly_cov), trans_matrix_3d.T)

        # Slice it to a smaller one
        i_slice = p1 * p_max ** 2 + p2 * p_max + p3
        analytic_cov = full_analytic_cov[i_slice,:][:,i_slice]
    ...
    def constrain_models(self, model_list, decomposition_coefficients=None, convergence_correlation=None, convergence_MSE=None, full_result=False):
        ...
        if full_result:
            ...
            map_no_values = np.tile(np.arange(n_maps), len(model_list))
